...
- [x] **v1.2 (2026-08-06): flows Renée specified.** Start screen = Purchase / Refinance / Renewal / Reverse / Private / Something else (engine honors explicit choice, same no-dead-end gates). Agent can fill intake for the client (their link then goes consent → straight to uploads). Welcome-back state for returning clients. Co-applicants: own private chase-link (default — docs private from primary, share toggle is THEIR choice) or delegated (primary uploads for them); applicant-aware slots/progress/nudges. 50 unit tests + 9-step live E2E.
